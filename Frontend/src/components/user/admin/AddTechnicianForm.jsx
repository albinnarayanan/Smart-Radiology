...
import { createNewTechnician } from '../../../store/actions';
import toast from 'react-hot-toast';
import { useDispatch } from 'react-redux';


const AddTechnicianForm = ({open, setOpen}) => {
    ...
    const handleCloseTechnicianForm = () =>{
        setOpen(false);
    }

    const saveTechnicianHandler = (data) =>{
        const sendData = {
                ...data,
            }
            
            dispatch(createNewTechnician(sendData, toast, reset, setLoader, setOpen));
  ...
  return (
    <div>
        ...
        <form onSubmit={handleSubmit(saveTechnicianHandler)} className='space-y-4'>
            <div className='flex items-center justify-center gap-4 '>
                <h2 className='flex w-full text-xl items-center justify-center font-semibold border'>
                    Fill the Technician details below
                </h2>
                <CloseButton onClick={handleCloseTechnicianForm} />
            </div>
